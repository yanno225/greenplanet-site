...
import { projectText, type Project } from "@/lib/projects";

type Props = { project: Project; prev: Project; next: Project };

export default function ProjectView({ project, prev, next }: Props) {
  const { t, lang } = useLang();
  const p = projectText(project, lang);
  const prevText = projectText(prev, lang);
  const nextText = projectText(next, lang);
      ...
      <section className="mx-auto w-full max-w-[1880px] px-6 pb-10 pt-6 md:px-8 md:pb-16 md:pt-10">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,46%)] lg:items-end lg:gap-16">
          <div>
            ...
            <h1 className="section-title mt-6 max-w-[16ch]">{p.heading}</h1>
            <p className="mt-8 max-w-[60ch] text-[18px] leading-relaxed text-ink-2">{p.intro}</p>
          </div>
          ...
          {p.delivered.length > 0 && (
            <div className="lg:pt-1">
              <div className="rounded-[24px] bg-gp-green p-8 text-white md:p-10">
                <h2 className="text-[clamp(22px,2vw,28px)] font-medium leading-tight tracking-tight">{t.projects.delivered}</h2>
      ...
      <section className="w-full border-t border-line bg-paper-2">
        <div className="mx-auto grid w-full max-w-[1880px] gap-6 px-6 py-10 md:grid-cols-2 md:px-8 md:py-14">
          <Link href={`/realisations/${prev.slug}`} className="group flex flex-col gap-2">
            <span className="text-[13px] uppercase tracking-[0.16em] text-ink-3">{t.projects.prev}</span>
          ...
          <Link href={`/realisations/${next.slug}`} className="group flex flex-col gap-2 md:items-end md:text-right">
            <span className="text-[13px] uppercase tracking-[0.16em] text-ink-3">{t.projects.next}</span>
            <span className="text-[clamp(20px,1.8vw,26px)] font-medium tracking-tight text-ink transition-colors group-hover:text-gp-green">
              {nextText.heading}
              <span aria-hidden> →</span>
            </span>
